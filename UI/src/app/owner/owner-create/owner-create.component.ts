...
export class OwnerCreateComponent implements OnInit {
  ...
  validateControl = (controlName: string) => {
    if(this.ownerForm.get(controlName).invalid && this.ownerForm.get(controlName).touched) return true;
  ...
  private executeOwnerCreation = (ownerFromValue) => {
    const apiUrl = 'api/owner';
    const owner: OwnerForCreation = {
      name: ownerFromValue.name,
      dateOfBirth: this.datePipe.transform(ownerFromValue.dateOfBirth, 'yyyy-MM-dd'),
      address: ownerFromValue.address
    }
